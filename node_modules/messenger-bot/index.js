...
class Bot extends EventEmitter {
  constructor (opts) {
    ...
    this.secondary_app_id = opts.secondary_app_id || false
  }
  ...
  sendHelpButtonTemplate (recipient, cb) {
    if (!cb) cb = Function.prototype
    request({
      method: 'POST',
        ...
        message: { 
          attachment: {
            ...
            payload: {
              ...
              text: 'Welcome to our store, please choose one of the following:',
              buttons: [
               /*
                {
                  type: 'web_url',
  ...
  passThreadControl (recipient, payload, cb) {
    if (!cb) cb = Function.prototype
    ...
    request({
      ...
      json: {
        recipient: { id: recipient },
        target_app_id: this.secondary_app_id,
        metadata : "start chat"
      }
    }, (err, res, body) => {
      if (err) return cb(err)
  ...
  _handleMessage (json) {
    ...
    entries.forEach((entry) => {
      ...
      if (events == null) 
      {
        ...
        {
        console.log('messaging is null in entry');
        }
        return;
      }
      events.forEach((event) => {
        ...
        if (event.pass_thread_control) {
          this._handleEvent('regain_control', event)
        }
